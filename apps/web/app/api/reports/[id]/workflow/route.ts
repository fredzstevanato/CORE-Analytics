...
import { z } from "zod";
import { transitionGeneratedReportWorkflow } from "@core/cases";
...
const bodySchema = z.object({
  action: z.enum(["SUBMIT_REVIEW", "APPROVE", "REOPEN_REVIEW"]),
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    await transitionGeneratedReportWorkflow({
      reportId: params.id,
      action: body.action,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Falha ao atualizar workflow do relatorio." },
      { status: 500 }
    );
  }
}
