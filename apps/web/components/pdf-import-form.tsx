    ...
    <div className="space-y-3">
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            <option value="analysis-and-ocr">analysis-and-ocr</option>
          </select>
        </div>

        <Button type="submit" disabled={loading}>
          {loading ? "Processando PDF..." : "Analisar PDF"}
        </Button>
      </form>
      ...
      {result ? (
        <div className="space-y-3 rounded-md border border-zinc-200 bg-zinc-50 p-3 text-sm">
          <p>
            <strong>Resumo:</strong> {result.summary.totalPages} paginas, {result.summary.pagesNeedingOcr} com OCR
            necessario, {result.summary.blankPages} em branco, {result.summary.possibleDuplicatePages} possiveis
            duplicadas.
          </p>
          <p>
          ...
          {result.processedFileUrl ? (
            <a className="text-emerald-700 underline" href={result.processedFileUrl} target="_blank" rel="noreferrer">
              Abrir PDF processado
            </a>
          ) : null}

          <div>
            <p className="font-medium">Paginas sem OCR util: {pagesWithoutOcr.map((page) => page.pageNumber).join(", ") || "-"}</p>
            <p className="font-medium">Paginas em branco: {blankPages.map((page) => page.pageNumber).join(", ") || "-"}</p>
          ...
          <div className="max-h-60 overflow-auto rounded border border-zinc-200 bg-white p-2">
            <p className="mb-2 font-medium">Analise por pagina</p>
